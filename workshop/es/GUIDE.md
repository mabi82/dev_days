<!-- l10n-sync: source-file="workshop/GUIDE.md" -->
# Soc Ops — VS Code GitHub Copilot Agent Lab

🌐 [English](../GUIDE.md) | [Danish](../da/GUIDE.md) | [Português (BR)](../pt_BR/GUIDE.md)

[🎮 Demo en Vivo](https://dotnet-presentations.github.io/vscode-github-copilot-agent-lab/) • [📚 Guía del Lab](https://dotnet-presentations.github.io/vscode-github-copilot-agent-lab/docs/) • [← README](../../README.es.md)

---

> **Guía de Referencia Rápida** — Para instrucciones detalladas, consulta las partes individuales a continuación.

---

## 📚 Partes del Lab

| Parte | Título | Tiempo |
|-------|--------|--------|
| [**00**](00-overview.md) | Descripción General & Lista Rápida | — |
| [**01**](01-setup.md) | Configuración & Ingeniería de Contexto | 15 min |
| [**02**](02-design.md) | Frontend Design-First | 15 min |
| [**03**](03-quiz-master.md) | Quiz Master Personalizado | 10 min |
| [**04**](04-multi-agent.md) | Desarrollo Multi-Agent | 20 min |

---

## ⚡ Lista Rápida

- [ ] VS Code **v1.107+**
- [ ] GitHub Copilot (Pro/Business/Enterprise)
- [ ] Git & .NET 10 SDK
- [ ] Panel de Chat abierto

---

## 🚀 Configuración Rápida

```
# 1. Usa el template en github.com/copilot-dev-days/agent-lab-dotnet
# 2. Activa GitHub Pages (Settings > Pages > GitHub Actions)
# 3. Clona en VS Code
# 4. Ejecuta /setup en el Chat
```

---

## 📋 Resumen de Tareas

### Parte 1: Ingeniería de Contexto
- [ ] Generar instrucciones del workspace
- [ ] Ejecutar background agent para linting
- [ ] Ejecutar cloud agent para README
- [ ] Revisar CSS e instrucciones de frontend

### Parte 2: Frontend Design-First  
- [ ] Rediseño completo de la UI (Plan Mode)
- [ ] Actualizar copilot-instructions.md
- [ ] Ejecutar cloud-explore para variaciones

### Parte 3: Quiz Master
- [ ] Generar tema de quiz personalizado
- [ ] Ejecutar Quiz Master como cloud agent

### Parte 4: Desarrollo Multi-Agent
- [ ] Modo Búsqueda del Tesoro (TDD Red → Green → Refactor)
- [ ] Modo Baraja de Cartas (Pixel Jam)
- [ ] Revisión de UX

---

## 🎭 Ideas de Temas

Minimalist Mono • Retro Terminal • Cyberpunk Neon • Vaporwave Sunset • Dark Mode Noir • Pixel Arcade • Space Galaxy • Brutalist Blocks • Soft Pastel • Gradient Glass

---

## 🎲 Temas de Quiz

Skill Bingo • Tech Life • Chaos Bingo • Travel • Fandom • Office Humor • Personality • Deep Chat • Opposites • Mystery

---

## 💡 Consejos

1. Mantén el navegador abierto para ver actualizaciones en vivo
2. Haz commit del código funcional con frecuencia  
3. Usa checkpoints para revertir errores
4. Itera en los planes 2+ veces antes de implementar

---

## 🔗 Recursos

- [VS Code YouTube](https://www.youtube.com/code)
- [Copilot Docs](https://code.visualstudio.com/docs/copilot/overview)
- [Awesome Copilot](https://github.com/github/awesome-copilot)
